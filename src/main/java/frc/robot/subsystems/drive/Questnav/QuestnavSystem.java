...
public class QuestnavSystem implements QuestnavIO {
    ...
    public void updateInputs(QuestnavIOInputs inputs) {
        ...
        inputs.batteryPercent = headset.getBatteryPercent();
        inputs.frameCount = headset.getFrameCount();
        inputs.tracking = headset.isTracking();
        inputs.trackingLostCount = headset.getTrackingLostCounter();

        inputs.isTrustworthy = (inputs.isConnected &&
                inputs.latency < 10 &&
                // inputs.frameCount > 60 &&
                inputs.tracking);
        ...
        if (poseFrames.length > 0) {
            ...
            inputs.estimatedPoseFieldCentric = estimatedRobotPose;
        } 
